...
object AmpleRuntimeBridge {
    private const val ENTRY_CLASS_NAME = "app.urv.manager.ample.runtime.AmpleRuntimeEntry"
    private const val CALLBACK_CLASS_NAME = "app.urv.manager.ample.runtime.AmpleRuntimeCallback"
    ...
    private fun ensureClassLoader(): DexClassLoader {
        val context = appContext ?: error("AmpleRuntimeBridge is not initialized.")
        val runtimeClassPathFile = AmpleRuntimeAssets.ensureRuntimeClassPath(context)
        ...
        synchronized(lock) {
            ...
            val optimizedDir = File(context.codeCacheDir, "ample-runtime-dex").apply { mkdirs() }
...
class AmpleBridgeFailureException(val originalStackTrace: String) :
    Exception("Ample in-process patcher failed")
