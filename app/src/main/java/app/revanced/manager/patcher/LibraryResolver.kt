...
abstract class LibraryResolver {
    ...
    protected fun findLibraryExact(context: Context, fileName: String): File? = File(context.applicationInfo.nativeLibraryDir).run {
        list { _, f -> !File(f).isDirectory && f == fileName }?.firstOrNull()?.let { resolve(it) }
    }
}
