...
[ -f "$state_file" ] || { log_status "Missing committed state; leaving stock active"; exit 0; }
...
. "$state_file"
[ "$URV_STATE_VERSION" = 1 ] || { log_status "Unsupported state version"; exit 0; }
...
installed_users="$(installed_user_ids)" || {
  log_status "Unable to verify Android user ownership; disabling the module"
